...
Golden test numbers: `tests/test_resolver.py::test_v5_golden` — the GAME_DESIGN.md §12 fixture (seed 42): Stabby 19, Blob 25, Lawnmower 38, Gerald 38 (v6 HP: max 32/33/40/38, Speed grants no HP). §12 lists the *actual* seed-42 dice, so doc and test agree; the test's narrative comment explains the round beat by beat. A companion test (`test_ko_before_slot_forfeits_the_tapped_action`) asserts a fighter KO'd before its initiative slot never resolves its tapped move.
