...
Tasks:
- `zones.py` / `moves.py` registries reading YAML generically (modifier lookup API per ARCHITECTURE.md §4.3). Every classified action resolves through its `moves.yaml` entry. *(A `conditions.py` registry existed in v1 and was deleted in v2.1 — see §7.)*
- `resolver.py`: *(v1 description — superseded by Combat v5 in §7; the current resolver has no AC/rolls/crits/dodge/conditions and every move lands, effect = stat base + flat creativity, with PROTECT's reflect shield the only reduction.)* PROTECT-first initiative, per-move effect formulas, creativity/stale modifiers, CHARGE/ESCAPE movement, KO/Gremlin traps, victory, sudden death.
...
6. **The audio layer:** curated free sound packs (CC0 — Kenney.nl, Mixkit, Freesound); `sfx` keys per move in moves.yaml; `events_sfx` stingers in settings.yaml (DEVASTATING → crowd roar, reflect → boing, trap springs → comic snap, KO → bell + gasp, combo → air horn, sudden death → drumroll); Web Audio manager with volume/mute and ±10% pitch variation.
