...
Tasks:
- `zones.py` / `moves.py` / `hazards.py` registries reading YAML generically (modifier lookup API per ARCHITECTURE.md §4.3). Every classified action resolves through its `moves.yaml` entry. *(A `conditions.py` registry was built here originally and deleted in v2.1 — see §7.)*
- `resolver.py`: *(v1 description — superseded by Combat v4 in §7; the current resolver has no AC/rolls/crits/conditions and every move lands, effectiveness = stat base + flat creativity, with passive dodge/shield.)* initiative, per-move effect formulas, creativity/stale modifiers, movement legality, KO/Gremlin, victory, sudden death.
...
Tasks:
- Player page: join form, team-colored theme, drawing canvas with pen (3 widths, 8 colors), **erasers in multiple sizes**, undo, clear; 512px PNG export, submit + auto-submit, status card (stats 💪/⚡/🌀 with change-pulse, HP hearts, condition emojis, round indicator), reconnect banner. *(Superseded in part by Combat v2 — see §7: move buttons + target picker now accompany the canvas.)*
...
- **Floating combat numbers:** spawn red (damage) / green (healing) numbers that float up and fade from the affected sprite on each damage/heal event (`float_number_seconds` config); DEVASTATING hits render oversized.
...
- `classify_actions`: *(as built in v1; superseded by Combat v2 — see §7 and GAME_DESIGN §11.1: tapped move + target arrive from the phone; the AI judges creativity, staleness, combos, TRICK conditions, and WILD CARD interpretation only.)*
...
7. **AI team names:** extend `generate_characters` prompt/schema + mock fixtures with a per-team name linking its roster (short, family-friendly, equally funny for both teams); teams are “Team A/B” in room state until then.
8. **Ongoing:** port `balance_sim` to run against the real engine/configs; mine `wildcards.jsonl` after playtests for new `moves.yaml` archetypes.
...
6. **The audio layer:** curated free sound packs (CC0 — Kenney.nl, Mixkit, Freesound); `sfx` keys per move in moves.yaml; `events_sfx` stingers in settings.yaml (DEVASTATING → crowd roar, dodge → whoosh, WILD backfire → sad trombone, KO → bell + gasp, combo → air horn, sudden death → drumroll); Web Audio manager with volume/mute and ±10% pitch variation.
7. **Lobby rules panels + status-card visibility:** "How to Play" panel on the host lobby (beside QR) and condensed rules on the player waiting screen (copy from GAME_DESIGN §13, editable in settings.yaml); phone status card hidden until the character exists — remove the empty sprite/dash-stats card from the lobby phase.
8. **Team-name reveal:** “…and TOGETHER they are…” final intro beat; all labels (zone bands, meter ends, phone headers) swap from Team A/B to the AI names and persist.
9. **Doodle crowd stands (S4):** rotating gallery spectators in the colosseum stands (subtle idle bob; never obscuring the battlefield).
