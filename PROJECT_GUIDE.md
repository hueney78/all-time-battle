...
- **v5.** Five single-target moves (SMASH/BLAST/CHARGE/ESCAPE/PROTECT), no AOE, **no dodge**. `HP = 27 + 2×POW + WRD + ⌊SPD/2⌋` (Speed given survivability once dodge was gone). PROTECT's reflect shield is the only damage reduction; it always acts first. Gremlins plant persistent zone traps on a blank canvas. Sim-verified rock-paper-scissors with no dump stat.
- **v6 — current.** Two balance/mechanic tweaks to curb Speed's first-strike dominance and give ESCAPE positional skill: (1) **Speed grants no HP** — `HP = 27 + 2×POW + WRD` (`hp_per_speed: 0`), so fast fighters trade tankiness for their initiative edge; (2) **ESCAPE is a parting shot** — you may tap any enemy, but it only hits one in the zone you fled from (`hits_from_zone_only`), else a `whiff`. See GAME_DESIGN §§3–5 for the authoritative rules.

**Settled — do not silently revert:** no status conditions; no attack rolls or AC; no AOE/multi-target; no "draw ahead" pipeline; announcers never reference mechanics; AI never chooses a player's move or target (the phone tap is ground truth); character names capped at two words; **every move lands except ESCAPE's positional parting shot** (v6); **Speed grants no HP** (v6).
