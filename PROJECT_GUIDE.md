...
- **Branching:** short-lived branches, PRs reviewed by the *other* person's Claude Code session against the specs ("review this diff against GAME_DESIGN.md/ARCHITECTURE.md; flag spec deviations, hardcoded values, and engine-purity violations"), CI (`pytest` + ruff) as the merge gate, one GitHub Issue per task.
