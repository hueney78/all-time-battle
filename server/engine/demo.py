...
def _print_event(ev: Event) -> None:
    ...
    if t == "attack_resolved":
        result = d.get("result", "?")
        if result in ("hit", "devastating"):
            pb = "  (point-blank ½)" if d.get("point_blank") else ""
            # The same addition the host readout shows the couch (§13).
            terms = f"dice={d.get('dice')} + {d.get('stat')}={d.get('stat_value')}"
...
def main() -> None:
    ...
    for ch in chars:
        print(f"  {ch.name:20s}  HP={ch.hp}/{ch.max_hp}"
