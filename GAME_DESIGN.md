...
The victory screen also offers a **“Start a New Match”** button beside the poster: it mints a **fresh room code** and reloads the host into a clean lobby — exactly like launching the server fresh, dropping the finished match's characters/HP/round state. (The persistent Doodle Crowd gallery, §15, carries over — it spans game nights by design.) Players re-scan the new QR to join the new room.
...
**Announcer length cap.** Every announcer line (a narration beat *and* a character intro) is held to `max_announcer_chars` (settings.yaml; `0` = no limit). The cap is both injected into the narrate prompt as a target *and* hard-enforced server-side — an over-long line is trimmed on a word boundary and given an ellipsis — so the booth never runs long on screen no matter what the model returns. Keep it in the low hundreds of characters to match the "1–3 sentences" comedy rule (§11.3).
...
- **Accidental-navigation guard:** once a match is underway, both the host TV and the player phones arm a browser `beforeunload` prompt, so a stray refresh, tab close, or back-gesture can't silently drop someone's place in the game. It is a native "Leave this page?" confirmation, not a custom dialog. The host's own **Start a New Match** button (§10.2) bypasses it intentionally.
...
- **Announcer voices (Text-to-Speech):** the host TV browser reads each revealed announcer beat **aloud** using the built-in Web Speech API, giving the duo a real voice — with a **different voice per announcer** (play-by-play vs color commentator). It's fully configured in `settings.yaml` (`ui.tts`: `enabled`, global `rate`/`pitch`/`volume`, and a per-announcer voice matched to an installed browser voice by `name` substring then `lang`, with per-announcer `pitch`/`rate` so the two stay distinct even on a machine with only one voice). Defaults bias play-by-play to a higher/faster voice and color to a lower/slower one; the TV browser console lists the installed voice names to copy into config. A **🗣️ toggle** on the host bar turns it on/off live, the commentary tracks the beat currently on screen (skipping ahead cuts the prior line), and it **degrades silently** where the browser has no speech synthesis. The awards ceremony is read aloud too (announcers alternating). TTS is the **shared screen's** job — the phones never speak.
...
Want faster games? Lower `hp_base`. Too swingy? Reduce `creativity_tier_3` from +5→+4. PROTECT too strong? Lower `reflect_per_weird` (5%→4%) or its cap. Kids losing? Raise `underdog_bonus`. Announcers too wordy on screen? Lower `max_announcer_chars` (§11.2). One move dominating playtests? Its whole formula is one line in moves.yaml. Every knob named in this doc exists in `balance.yaml` with a comment. Change YAML → start a new room → new rules apply. After each game night, skim the **flavor log** (`snapshots/<room>/flavor.jsonl`) — recurring notes about drawings that didn't fit any of the five moves are your signal for what the catalog might be missing.
